  ...
  def __init__(self,
    ...
    if not label_lookup_path:
      label_lookup_path = os.path.join(
          app_path, 'imagenet_2012_challenge_label_map_proto.pbtxt')
    if not uid_lookup_path:
      uid_lookup_path = os.path.join(
          app_path, 'imagenet_synset_to_human_label_map.txt')
...
def create_graph():
  ...
  with tf.gfile.FastGFile(os.path.join(
      app_path, 'classify_image_graph_def.pb'), 'rb') as f:
...
def run_inference_on_image(image):
  ...
  with tf.Session() as sess:
    ...
    for node_id in top_k:
      ...
      print('%.1f%%: %s' % (score*100,human_string))
      RESULTS_ANALYSIS=RESULTS_ANALYSIS+'%.1f%%: %s' % (score*100,human_string)+';'
